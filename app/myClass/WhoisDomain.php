...
class WhoisDomain
{
    ...
    {
        $this->domain = trim(strip_tags($domain));
        $this->server_whois = $this->getServerWhois();
    }

    public function getInfo(bool $line = false)
    {

        $connect = @fsockopen($this->server_whois, 43, $errno, $errstr, 10);

        if (!$connect) {
            return "ERROR: $errno - $errstr";
        }

        fwrite($connect, $this->domain . "\r\n");
        $whois = fread($connect, 8192);
        fclose($connect);
    ...
    public function getExpiryDate()
    {
        ...
        foreach ($this->getInfo(true) as $key => $value) {
            if (preg_match('/(expiry)/i', $value)) {
                $items = explode(':', $value);
                unset($items[0]);
    ...
    public function getRegistrar()
    {
        ...
        foreach ($this->getInfo(true) as $key => $value) {
            if (preg_match('/(registrar:)/i', $value)) {
                $items = explode(':', $value);
                unset($items[0]);
                return trim(implode(':', $items));
            }
    ...
    public function getServerDNS()
    {
        ...
        $dns = [];
        foreach ($this->getInfo(true) as $key => $value) {
            if (preg_match('/(name server)/i', $value)) {
                $items = explode(':', $value);
                unset($items[0]);
                $dns[] = strtolower(trim(implode(':', $items)));
            }
        }
    ...
    public function setServerWhois(string $server)
    {
        $this->server_whois = $server;
    }

    public function getServerWhois() //: string
    {
